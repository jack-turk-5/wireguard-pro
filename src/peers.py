...
def create_peer(days_valid=7):
    ...
    # 2) Allocate next free IPv4/IPv6
    ...
    # 5) Append to the on-disk WireGuard config
    append_peer_to_wgconf(pub, ipv4, ipv6)

    # 6) Inject into the running interface without full reload
    run([
        "wg", "set", "wg0",
        "peer", pub,
...
def delete_peer(public_key):
    ...
    # 1) Filter out the peer stanza from the on-disk config
    ...
    # 2) Write the filtered config back
    with open(WG_PATH, "w") as f:
        f.write("\n".join(new_lines) + "\n")

    # 3) Remove the peer from the live interface
